...
> Nearli 不是行程管理、不是 Google Calendar、不是 Google Maps。
> Nearli 陪你完成一次出門：現在，這一件事。做完，下一件事自然出現。

品牌名稱：**Nearli**（中文概念：快到了）。顯示名稱與文案統一放在 `src/config/app.ts`，各頁面一律從這裡引用，不會寫死文字。

**v0.1.11**：修正兩個視覺問題——吉祥物身體改成正圓形（之前誤做成蛋形／橢圓形）；任務跑道的格子加上立體感（上面＋斜切的側邊）與高低錯落（離目前這一步越遠的格子微微墊高），呼應參考圖裡「路是斜的」透視感。這是用 CSS 變形（`skewX`、`translateY`）模擬出來的立體效果，不是真正的等角（isometric）3D 或遊戲引擎算圖，所以不會跟參考截圖的立體方塊完全一樣，但整體感覺更接近了。純視覺，沒有動任何資料或邏輯。

**v0.1.10**：首頁的準備流程加入遊戲風格的「任務跑道」——一排格子，長短依照每個準備事項的預估分鐘數分配寬度，吉祥物小球沿著格子頂端慢慢移動，完成一項、按下按鈕後，小球會滑到下一格。格子上顯示事項名稱與小圖示，下方顯示分鐘數。同時在準備／出發畫面頂部加回一句問候與抵達時間（例如「早安，週六約會・18:00 抵達漢神巨蛋」），呼應使用者提供的參考畫面。純視覺與呈現層變動（新增 `src/lib/prepIcons.tsx`、`TaskCarousel` 元件），沒有動任何資料或商業邏輯。

**v0.1.8**：把 Nearli 的靈感從「行程管理」徹底移到「出門」本身。首頁在還不用準備時，改成安靜的陪伴訊息（「現在不用準備。距離開始還有 23 小時。Nearli 會提醒你。」），不再預告任務名稱；一旦該做事了，畫面統一用「現在／{動作}」的模式呈現（洗澡、出門、前往下一站都套用同一種語言），並加入一條安靜的進度條表示還剩多久。行程詳情的時間軸從「時程表」改成「出門清單」——打勾／實心圈／空心圈的節點序列（洗澡✓ 換衣服✓ 收包包○ 出門○ 抵達○），時間退成每個節點旁邊的小字附註。新增行程的手動流程拆成五個更純粹的單一問題（旅程叫什麼？→哪一天？→第一站去哪？→出門前要做什麼？→完成），一次只問一件事。這次沒有調整圓角、配色、陰影或其他 CSS 細節，只重新設計了畫面結構、資訊層級與互動流程；資料結構、Storage、Migration、Google Maps、AI、Learning Record、Review 邏輯完全沒有變動。詳見 [`docs/PROJECT_MAP.md`](./docs/PROJECT_MAP.md)。
