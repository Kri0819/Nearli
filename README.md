...
> Nearli 不是導航，不是行事曆，也不是管理工具。
> 它只做一件事：告訴你現在該做什麼，做完之後，再自然地帶你到下一步。

品牌名稱：**Nearli**（中文概念：快到了）。顯示名稱與文案統一放在 `src/config/app.ts`，各頁面一律從這裡引用，不會寫死文字。

**v0.1.7**：重新定義 Design Language——參考 Apple Health / Fitness / Journal / Reminders 這類「陪伴型」App，而不是後台 Dashboard。首頁改成留白為主的 Hero 頁面（沒有卡片邊框，只有一個大標題、一個主要按鈕）；行程列表改成垂直時間軸（節點＋連接線），不再是一疊白色卡片；新增行程的手動流程改成一步一步的精靈（哪一天？→第一站去哪？→出門前要做什麼？→完成），不再一次出現所有欄位；行程回顧改成大按鈕的對話式問答；設定頁改成貼近 iOS 設定頁的分隔線列表，拿掉多餘的卡片邊框與陰影。這次只改 UX／UI／版面／動效／字級／元件設計，沒有新增功能、沒有改資料結構、沒有動到 Google Maps、AI、Learning Record、Storage、Migration 或時間計算。詳見 [`docs/PROJECT_MAP.md`](./docs/PROJECT_MAP.md) 的版本紀錄與設計說明。
