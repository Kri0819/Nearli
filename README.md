...
**v0.1.6**：重新定義首頁的產品體驗——大標題改成「現在該做的事」（例如「現在去洗澡吧。」），時間退為輔助資訊；拿掉首頁上的抵達時間、交通時間、停車時間、步行時間（這些移到「查看完整行程」，首頁只留現在做什麼、還剩多久、下一步）；按鈕文案依任務調整（洗好了／換好了／我出發了／我到了），不再一直出現「開始這一步」「完成，下一步」；行程回顧文案改成更口語的「今天哪裡最花時間？」。這次只改文案與呈現方式，時間計算、資料結構、Learning Record、Maps、AI 全部沒有變動。
